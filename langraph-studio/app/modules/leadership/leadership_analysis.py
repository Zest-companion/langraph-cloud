...
def leadership_vector_search(state: WorkflowState) -> Dict[str, Any]:
    ...
    try:
        ...
        leadership_resources = _format_leadership_results(leadership_results, question_type, detected_styles)
...
def leadership_analysis(state: WorkflowState) -> Dict[str, Any]:
    ...
    Tu es le coach ZEST COMPANION.
    Pour les questions spécifiquement sur le MBTI, redirige vers le chatbot dans la section "Better understand and manage myself" intitulé "Explore my personality with MBTI".
    Contacte votre directeur de programme académique à jean-pierre.aerts@zestforleaders.com pour toute question plus poussée (Contact your academic program director at jean-pierre.aerts@zestforleaders.com for any advanced questions)."""
...
def _execute_goleman_search(query: str, question_type: str, detected_styles: list, state: dict) -> list:
    ...
    try:
        ...
        if strategy.get('two_stage_search', False):
            ...
            # Stage 5: Vector search enrichment for personal_style
            logger.info("🔍 Stage 5 - Vector search enrichment for personal context")
            try:
                from ..lencioni.lencioni_analysis import perform_supabase_vector_search
                ...
                if folder_path:
                    ...
                    # Check if we have high-quality results (similarity > 0.5)
                    ...
                    if not high_quality_results:
                        logger.info("⚠️ No high-quality prioritized results (similarity > 0.5), trying non-prioritized")
                        vector_results = perform_supabase_vector_search(
                            query=query,
                            match_function='match_documents',
                            metadata_filters={
                        ...
                        # Filter again for quality results
                        high_quality_results = [
                            item for item in vector_results 
                            if item.get('similarity', 0.0) > 0.5
                        ]
                    ...
                    logger.info(f"🔍 Stage 5 - Added {len(high_quality_results)} high-quality vector results")
                else:
                    logger.warning("⚠️ No folder_path available for vector search enrichment")
                    
            except Exception as e:
                logger.error(f"❌ Stage 5 vector search enrichment failed: {e}")
            ...
            logger.info(f"✅ Multi-stage retrieval with vector enrichment completed: {len(results)} total items")
            ...
            # 3. Add vector search for additional context (same logic as personal_style)
            logger.info("🔍 Adding vector search enrichment for comparative context")
            try:
                from ..lencioni.lencioni_analysis import perform_supabase_vector_search
            ...
            # Process ALL target styles (no limit)
            if target_styles:
                logger.info(f"🎯 Implementation search for styles: {target_styles}")
                
                # Process all styles without limit
                styles_to_process = target_styles
                
                for target_style in styles_to_process:
                    logger.info(f"🔍 Processing implementation for '{target_style}' style")
                    
                    # 1. Priority: Practical Templates (massively prioritized)
                    logger.info(f"🛠️ Priority search: Practical templates for {target_style}")
                    try:
                        practical_response = supabase.table('documents_content_test').select('content, metadata').eq(
                            ...
                            'metadata->>leadership_style', target_style
                        ).limit(3).execute()  # Reduced per style since we handle multiple
                        ...
                        logger.info(f"🛠️ Retrieved {len(practical_response.data)} practical templates for {target_style}")
                        
                    except Exception as e:
                        logger.error(f"❌ Failed to retrieve practical templates for {target_style}: {e}")
                    
                    # 2. Complementary: Style Complete (conceptual understanding)
                    logger.info(f"📊 Complementary search: Style complete for {target_style}")
                    try:
                        conceptual_response = supabase.table('documents_content_test').select('content, metadata').eq(
                            ...
                            'metadata->>leadership_style', target_style
                        ).limit(1).execute()  # Reduced per style
                        ...
                        logger.info(f"📊 Retrieved {len(conceptual_response.data)} conceptual complements for {target_style}")
                        
                    except Exception as e:
                        logger.error(f"❌ Failed to retrieve conceptual complements for {target_style}: {e}")
            
            # 3. Temperament leadership style - Direct retrieval (same as personal_style)
            user_temperament = state.get('user_temperament', '')
            if user_temperament:
                logger.info(f"🧬 Stage 3 - Temperament leadership style: {user_temperament}")
                try:
                    ...
                    logger.info(f"🧬 Added {len(stage3_response.data)} temperament leadership insights")
                    
                except Exception as e:
                    logger.error(f"❌ Stage 3 temperament retrieval failed: {e}")
            
            # 4. Vector Search Enrichment (same as other types)
            logger.info("🔍 Adding vector search enrichment for implementation context")
            try:
                from ..lencioni.lencioni_analysis import perform_supabase_vector_search
                ...
                logger.error(f"❌ Failed to execute implementation vector search: {e}")
            
            logger.info(f"🛠️ Implementation search with vector enrichment completed: {len(results)} total items")
            return results
            ...
            # Temperament leadership style - Direct retrieval (same as personal_style and implementation)
            user_temperament = state.get('user_temperament', '')
            if user_temperament:
                logger.info(f"🧬 Stage 3 - Temperament leadership style: {user_temperament}")
                try:
            ...
            # VECTOR SEARCH ENRICHMENT: Add high-quality vector search for additional context (harmonized with other types)
        ...
        logger.error(f"❌ Error in _execute_goleman_search: {e}")
        return []
...
"""
        ...
        response_content = isolated_analysis_call_with_messages(
            system_content="You are the ZEST COMPANION coach, a Goleman leadership expert. For MBTI questions, redirect to 'Explore my personality with MBTI' chatbot. Contact jean-pierre.aerts@zestforleaders.com for advanced questions. Respond only with strict JSON.",
            user_content=prompt
        )
...
def _format_leadership_results(results: list, question_type: str, detected_styles: list = None) -> str:
    """Formatter les résultats selon le type de question"""
    if not results:
        return ""
    ...
    if question_type == 'comparative':
        ...
        return _format_implementation_results(results, detected_styles)
    elif question_type == 'personal_style':
        return _format_personal_style_results(results)
...
def _format_comparative_results(results: list) -> str:
    ...
    for result in results:
        ...
        if is_vector_search or target_type == 'vector_context_comparative':
    ...
    # Add high-quality vector search context (supplementary information - limit to 2)
    if vector_results:
        formatted_output += "\n\n📋 **Additional Context**\n"
        formatted_output += "Supplementary information highly relevant to your comparison:\n\n"
        
        # Sort by similarity score (highest first) and take only top 2
        sorted_vector_results = sorted(vector_results, 
                                     key=lambda x: x.get('similarity', 0.0), 
                                     reverse=True)[:2]  # Limit to 2 results
        
        for i, result in enumerate(sorted_vector_results, 1):
            content = result.get('content', '')
...
def _format_implementation_results(results: list, detected_styles: list = None) -> str:
    """
    Special formatting for implementation questions with proper sequencing:
    1. Instructions for implementation
    2. Practical "how-to" templates by style
    3. Conceptual "why it works" understanding
    4. Vector enrichment insights
    """
    if not results:
        return ""
    
    # Organize results by search type and style
    practical_by_style = {}
    conceptual_by_style = {}
    vector_results = []
    styles_involved = set()
    
    # Track if user originally had specific style intent
    user_had_specific_styles = detected_styles and len(detected_styles) > 0
    
    for result in results:
        search_type = result.get('search_type', '')
        target_style = result.get('target_style', '')
        metadata = result.get('metadata', {})
        is_vector = result.get('vector_search', False)
        
        if target_style:
            styles_involved.add(target_style)
        elif metadata.get('leadership_style'):
            styles_involved.add(metadata['leadership_style'])
            
        if is_vector or search_type == 'vector_enrichment':
            vector_results.append(result)
        elif search_type == 'practical_priority':
            style = target_style or metadata.get('leadership_style', 'unknown')
            if style not in practical_by_style:
                practical_by_style[style] = []
            practical_by_style[style].append(result)
        elif search_type == 'conceptual_complement':
            style = target_style or metadata.get('leadership_style', 'unknown')
            if style not in conceptual_by_style:
                conceptual_by_style[style] = []
            conceptual_by_style[style].append(result)
    
    formatted_output = ""
    
    # Header and approach based on user's original intent
    if user_had_specific_styles:
        # User mentioned specific styles - provide targeted implementation
        if len(styles_involved) > 1:
            styles_list = ', '.join([s.title() for s in sorted(styles_involved)])
            formatted_output += f"# 🛠️ **Implementation Guide for Multiple Leadership Styles**\n"
            formatted_output += f"*Implementing: {styles_list}*\n\n"
        elif len(styles_involved) == 1:
            style = list(styles_involved)[0]
            formatted_output += f"# 🛠️ **How to Implement {style.title()} Leadership**\n\n"
        else:
            formatted_output += f"# 🛠️ **Leadership Implementation Guide**\n\n"
        specific_implementation = True
    else:
        # User asked generic "improve my leadership" - help them choose first
        formatted_output += f"# 🛠️ **Leadership Implementation Guide**\n\n"
        specific_implementation = False
    
    # Add implementation instructions based on context
    if specific_implementation:
        # Specific style(s) identified - provide targeted implementation guidance
        formatted_output += "## 📋 **HOW TO USE THIS IMPLEMENTATION GUIDE**\n\n"
        formatted_output += "**Your task:** Synthesize the practical guides below into a clear implementation plan.\n\n"
        formatted_output += "**The templates below contain for each style:**\n"
        formatted_output += "• **When to Use** - Specific situations where this style works\n"
        formatted_output += "• **How to Use It** - Concrete phrases and actions\n"
        formatted_output += "• **Benefits** - Expected positive outcomes\n"
        formatted_output += "• **Pitfalls** - What to avoid\n\n"
        formatted_output += "**Structure your response to highlight:**\n\n"
        formatted_output += "1. **Key implementation points** from the guides:\n"
        formatted_output += "   - Start with the 'How to Use' actions\n"
        formatted_output += "   - Emphasize the specific phrases and behaviors\n"
        formatted_output += "   - Highlight the pitfalls to avoid\n"
        formatted_output += "2. **Underlying competencies** (from conceptual foundations):\n"
        formatted_output += "   - Emotional intelligence skills that make this work\n"
        formatted_output += "   - Why these approaches are effective\n\n"
        formatted_output += "**Important:** Don't invent timelines or plans. Use the structured guidance already provided in the templates.\n\n"
    else:
        # No specific style - introduce all available styles
        formatted_output += "## 📚 **LEADERSHIP STYLES OVERVIEW**\n\n"
        formatted_output += "**Your task:** Present all available leadership styles to help the user choose.\n\n"
        formatted_output += "**Structure your response as follows:**\n\n"
        formatted_output += "1. **Introduction**\n"
        formatted_output += "   - Explain that multiple leadership styles are available for implementation\n"
        formatted_output += "   - Each style has specific strengths and optimal situations\n\n"
        formatted_output += "2. **Style Summaries** (from templates below)\n"
        formatted_output += "   For each style, provide:\n"
        formatted_output += "   - **Name and motto** (e.g., Authoritative: \"Come with me\")\n"
        formatted_output += "   - **When to use** (key situations)\n"
        formatted_output += "   - **Main benefits** (expected outcomes)\n"
        formatted_output += "   - **Best for** (type of leader/situation)\n\n"
        formatted_output += "3. **Implementation Note**\n"
        formatted_output += "   - Invite user to specify which style(s) they want to implement\n"
        formatted_output += "   - Explain that detailed implementation guidance is available once they choose\n\n"
    
    formatted_output += "---\n\n"
    
    # 1. PRACTICAL TEMPLATES BY STYLE
    if practical_by_style:
        formatted_output += "## 📚 **SOURCE MATERIAL: Practical Templates**\n"
        formatted_output += "_Use these templates to build your implementation plan:_\n\n"
        
        for style, style_practicals in practical_by_style.items():
            if len(practical_by_style) > 1:
                # Get style metadata
                metadata = style_practicals[0].get('metadata', {})
                climate_impact = metadata.get('climate_impact', 'neutral')
                style_motto = metadata.get('style_motto', '').replace('_', ' ').title()
                impact_emoji = "⬆️" if climate_impact == "highly_positive" else "✅" if climate_impact == "positive" else "⚠️" if climate_impact == "negative" else "➡️"
                
                formatted_output += f"### {impact_emoji} **{style.title()} Style**"
                if style_motto:
                    formatted_output += f" - \"{style_motto}\"\n"
                else:
                    formatted_output += "\n"
            
            for i, result in enumerate(style_practicals, 1):
                content = result.get('content', '')
                formatted_output += f"**Step {i}:**\n{content}\n\n"
    
    # 2. CONCEPTUAL UNDERSTANDING BY STYLE
    if conceptual_by_style:
        formatted_output += "## 📊 **SOURCE MATERIAL: Conceptual Foundations**\n"
        formatted_output += "_Understanding the emotional intelligence and mechanisms behind each style:_\n\n"
        
        for style, style_conceptuals in conceptual_by_style.items():
            if len(conceptual_by_style) > 1:
                formatted_output += f"### **{style.title()} - Core Mechanisms**\n"
            
            for result in style_conceptuals:
                content = result.get('content', '')
                formatted_output += f"{content}\n\n"
    
    # 3. VECTOR ENRICHMENT (limit to 2)
    if vector_results:
        formatted_output += "## 💡 **SOURCE MATERIAL: Contextual Insights**\n"
        formatted_output += "_Additional relevant content from your knowledge base:_\n\n"
    ...
    # Temperament-Based Leadership Tendencies (same as personal_style)
    temperament_leadership = [result for result in results 
                             if result.get('metadata', {}).get('facet') == 'leadership_style']
    if temperament_leadership:
        formatted_output += "# 🧬 **Your Natural Leadership Tendencies**\n"
        formatted_output += "How your temperament influences your leadership implementation approach:\n\n"
...
def _format_personal_style_results(results: list) -> str:
    ...
    Organizes: research foundation → integration principles → temperament → detected styles analysis → vector context
    """
    if not results:
        return ""
    ...
    for result in results:
        ...
        if is_vector_search:
            vector_enrichment.append(result)
        elif content_type == 'research_foundation':
    ...
    # 5. Contextual Insights (Vector Search Enrichment)
    if vector_enrichment:
        formatted_output += "# 💡 **Contextual Leadership Insights**\n"
        formatted_output += "Additional relevant insights based on your current context:\n\n"
        
        # Sort by similarity score (highest first) and take only top 2
        sorted_vector_results = sorted(vector_enrichment, 
                                     key=lambda x: x.get('similarity', 0.0), 
                                     reverse=True)[:2]  # Limit to 2 results
...
def _format_situational_results(results: list) -> str:
    ...
    if llm_detected_flags:
        formatted_output += f"# 🤖 **Situational Leadership Analysis** (AI-Enhanced)\n"
        formatted_output += f"*Leadership styles identified through AI analysis: {', '.join(set(llm_detected_flags)).title()}*\n\n"
    elif fallback_flags:
        formatted_output += f"# 🔄 **Comprehensive Situational Leadership Analysis**\n"
        formatted_output += f"*Evaluating all leadership styles for your situation*\n\n"
    else:
        styles_list = ', '.join([s.title() for s in styles_involved if s != 'unknown'])
        formatted_output += f"# 🎯 **Situational Leadership Analysis**\n"
        if styles_list:
            formatted_output += f"*Analyzing: {styles_list} Leadership approaches*\n\n"
    
    # Add explicit instruction for situational analysis
    formatted_output += "## 📊 **KEY INSTRUCTION FOR SITUATIONAL ANALYSIS**\n"
    formatted_output += "Based on the following information, provide:\n\n"
    formatted_output += "**First, briefly introduce each relevant leadership style:**\n"
    formatted_output += "- Name and motto (e.g., Authoritative: \"Come with me\")\n"
    formatted_output += "- Core characteristic in 1-2 sentences\n"
    formatted_output += "- Climate impact (positive/negative)\n\n"
    formatted_output += "**Then, for each style explain:**\n"
    formatted_output += "1. **WHEN** to use it (specific situations/contexts)\n"
    formatted_output += "2. **WHY** it's appropriate (benefits and expected outcomes)\n"
    formatted_output += "3. **HOW** to recognize when this style fits\n"
    formatted_output += "4. **RISKS** to avoid when applying it\n\n"
    formatted_output += "**Finally, provide a clear recommendation:**\n"
    formatted_output += "- Which style(s) best fit the user's specific situation\n"
    formatted_output += "- Rationale for this recommendation\n"
    formatted_output += "- Potential style combinations if relevant\n\n"
    
    # PRIORITY 1: PRACTICAL TEMPLATES (80% weight - immediate actionable guidance)
    if practical_templates:
        formatted_output += "## 🛠️ **PRACTICAL TEMPLATES - When & How to Apply**\n"
        formatted_output += "Specific approaches showing WHEN each style works best:\n\n"
    ...
    if style_completes:
        formatted_output += "## 🧠 **WHY THESE STYLES WORK - Understanding the Mechanisms**\n"
        formatted_output += "The psychological and organizational reasons WHY each style is effective in specific situations:\n\n"
    ...
    if situational_contexts:
        formatted_output += "## 📖 **SITUATIONAL INDICATORS - Recognizing When to Use Each Style**\n"
        formatted_output += "Key signals and contexts that indicate WHEN each leadership style is most appropriate:\n\n"
    ...
    # VECTOR SEARCH ENRICHMENT: Additional contextual insights (limit to 2)
    if vector_contexts:
        ...
        # Sort by similarity score (highest first) and take only top 2
        sorted_vector_contexts = sorted(vector_contexts, 
                                      key=lambda x: x.get('similarity', 0.0), 
                                      reverse=True)[:2]  # Limit to 2 results
        
        for i, result in enumerate(sorted_vector_contexts, 1):
            content = result.get('content', '')
            similarity = result.get('similarity', 0.0)
            metadata = result.get('metadata', {})
            
            # Get document info if available
            source = metadata.get('source_file', metadata.get('title', 'Contextual Insight'))
            formatted_output += f"**Insight {i}** (Relevance: {similarity:.2f})\n"
            formatted_output += f"*Source: {source}*\n\n"
            formatted_output += f"{content}\n\n"
    
    # Temperament-Based Leadership Tendencies (same as personal_style and implementation)
    temperament_leadership = [result for result in results 
                             if result.get('metadata', {}).get('facet') == 'leadership_style']
    if temperament_leadership:
        formatted_output += "# 🧬 **Your Natural Leadership Tendencies**\n"
        formatted_output += "How your temperament influences your situational leadership approach:\n\n"
