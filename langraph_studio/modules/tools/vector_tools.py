...
def fetch_user_profile(state: WorkflowState) -> WorkflowState:
    ...
    logger.info(f"🔍 DEBUG fetch_user_profile - main_theme: {state.get('main_theme')}")
    logger.info(f"🔍 DEBUG fetch_user_profile - sub_theme: {state.get('sub_theme')}")
    logger.info(f"🔍 DEBUG fetch_user_profile - message count: {len(state.get('messages', []))}")
